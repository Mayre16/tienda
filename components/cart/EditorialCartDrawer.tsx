...
import { Loader2, Mail, Minus, Plus, Trash2, X } from "lucide-react";
import {
  useCartActions,
  useCartItemsState,
  useCartOpen,
...
import { cmsEditorOrigin } from "@/lib/cms/api-client";
import { submitTiendaPedido } from "@/lib/submit-tienda-pedido";
...
function EditorialCartDrawerPanel() {
  ...
  function validateCustomer(): string | null {
    ...
    if (!customer.name.trim() || !customer.email.trim()) {
      return "Indique su nombre y correo electrónico.";
    }
    if (!customer.phone.trim()) {
      return "Indique teléfono o WhatsApp.";
    }
    return null;
  }
  ...
  async function handleRequestPaymentLink() {
    ...
    setBusy(true);
    try {
      const result = await submitTiendaPedido({
        customer,
        items,
        note,
        turnstileToken,
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        {items.length === 0 && !success ? (
          <p className="text-sm text-na-muted">
            Aún no hay artículos. Explore el catálogo y use «Añadir al carrito»
            en libros impresos y regalos con precio.
          </p>
        ) : items.length === 0 && success ? (
          <p className="rounded-lg bg-emerald-50 px-3 py-3 text-sm text-emerald-900">
            {success}
          </p>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {item.description ? (
                      <p className="mt-0.5 text-xs text-na-muted line-clamp-2">
                        {item.description}
                      </p>
                    ) : null}
      ...
      <div className="border-t border-na-editorial/10 px-5 py-4">
        <div className="space-y-3">
          ...
          <label className="block text-sm">
            <span className="font-semibold text-na-ink">Nota (opcional)</span>
            <textarea
              value={note}
              onChange={(e) => setNote(e.target.value)}
              rows={2}
              className="mt-1 w-full rounded-lg border border-na-editorial/20 px-3 py-2 text-sm"
              placeholder="Ej. recogida en jornada, sede, etc."
            />
          </label>
        </div>
